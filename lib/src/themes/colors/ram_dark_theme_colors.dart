import 'package:yod_nak_ram_ui_kit/src/themes/colors/color_tokens.dart';
import 'package:yod_nak_ram_ui_kit/src/themes/colors/ram_theme_colors.dart';

final ramDarkThemeColors = RamThemeColors(
  black: kColorBlack100,
  white: kColorWhite100,
  whiteBlack: kColorBlack100,
  blackWhite: kColorWhite100,
  blackWhiteOverlay: kColorWhite10,
  mainOverlay: kColorBlack70,
  imageOverlay: kColorBlack60,
  neutral: kColorNeutral70,
  transparent: kColorWhite0,
  //
  bgBg: kColorNeutral15,
  bgContainer: kColorNeutral25,
  bgContainerSection: kColorNeutral35,
  bgBorderContainer: kColorNeutral45,
  bgContainerDisable: kColorNeutral40,
  bgBorderDisable: kColorNeutral40,
  //
  textIconText: kColorNeutral95,
  textIconMidText: kColorNeutral80,
  textIconSubdued: kColorNeutral70,
  textIconOnOverlay: kColorNeutral10,
  textIconDisable: kColorNeutral60,
  textIconError: kColorDanger70,
  textIconPrimary: kColorPrimary80,
  textIconWarning: kColorWarning70,
  //
  primaryPrimary: kColorPrimary80,
  primarySurface: kColorPrimary10,
  primaryPressed: kColorPrimary50,
  primaryOnPrimary: kColorPrimary10,
  primaryOnSurface: kColorPrimary90,
  primaryBorder: kColorPrimary20,
  //
  secondarySecondary: kColorSecondary40,
  secondarySurface: kColorSecondary60,
  secondaryPressed: kColorSecondary20,
  secondaryOnSecondary: kColorSecondary95,
  secondaryOnSurface: kColorSecondary90,
  secondaryBorder: kColorSecondary60,
  //
  successSuccess: kColorSuccess60,
  successOnSurface: kColorSuccess40,
  successSurface: kColorSuccess99,
  successOnSuccess: kColorSuccess99,
  successPressed: kColorSuccess40,
  successBorder: kColorSuccess90,
  //
  warningWarning: kColorWarning70,
  warningSurface: kColorWarning99,
  warningPressed: kColorWarning50,
  warningOnWarning: kColorWarning10,
  warningOnSurface: kColorWarning20,
  warningBorder: kColorWarning80,
  //
  errorError: kColorDanger60,
  errorSurface: kColorDanger99,
  errorPressed: kColorDanger30,
  errorOnError: kColorDanger99,
  errorOnSurface: kColorDanger30,
  errorBorder: kColorDanger90,
  //
  notificationNotification: kColorDanger50,
  //
  elevationWeight1: kColorBlack20,
  elevationWeight2: kColorBlack70,
  staticHighlight: kColorPrimary95,
  //
  skeletonBase: kColorNeutral25,
  skeletonHighlight: kColorNeutral35,
);
